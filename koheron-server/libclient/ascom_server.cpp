...
uint32_t Revu24str2long(char *a) {
  char *s = a + 3;
  size_t z = Commands_getLength(a[1], 1);
...
{
  size_t z = Commands_getLength(buffer[1], 1);
	if (z > -1) buffer[z + 3] = '\0'; 
}
...
int main(int argc, char const *argv[]) {
  ...
  client->call<op::SkyTrackerInterface::override_steps_per_rotation>((uint8_t)1, (uint32_t)0xFFFFFF);
  client->recv<op::SkyTrackerInterface::override_steps_per_rotation, bool>();
  client->call<op::ASCOMInterface::SwpSetAxisPosition>((uint8_t)1, (uint32_t)0x800000);
  client->recv<op::ASCOMInterface::SwpSetAxisPosition, bool>();
  client->call<op::ASCOMInterface::SwpSetAxisPosition>((uint8_t)1, (uint32_t)0x800000);
  client->recv<op::ASCOMInterface::SwpSetAxisPosition, bool>();
  */
  bool isSlew[2] = {false};
  bool isForward[2] = {false};
  bool isIncrement[2] = {false};
  for (;;) {
   char command[64];
    ...
    if (buffer[0] == ':') {
      ...
      switch (buffer[1]) {
        ...
        case 'e': {
          ...
          klog << "InquireMotorBoardVersion " << resp << " " << axis
               << std::endl;
          synta_assembleResponse(command, buffer[1], resp);
          send(new_socket, command, strlen(command), 0);
          break;
        ...
        case 'g': {
          ...
          klog << "InquireHighSpeedRatio " << HEX(buffer[2]) - 1 << " " <<resp
               << std::endl;
          synta_assembleResponse(command, buffer[1], resp);
          send(new_socket, command, strlen(command), 0);
          break;
        ...
        case 'b': {
          ...
          resp = client->recv<op::ASCOMInterface::SwpGetTimerInterruptFreq,
                              uint32_t>();
          klog << "InquireTimerInterruptFreq Axis" << HEX(buffer[2]) - 1 << " " << resp
               << std::endl;
        ...
        case 'P': {
          uint32_t resp = 0;
          klog << "SetST4GuideRateCmd Axis" << HEX(buffer[2]) - 1 << " " << resp << std::endl;
          synta_assembleResponse(command, buffer[1], resp);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        ...
        case 'q': {
          ...
          klog << "GetFeatureCmd Axis " << HEX(buffer[2]) - 1 << " " << resp << std::endl;
          resp = 1 << 3 ;
          synta_assembleResponse(command, buffer[1], resp);
          send(new_socket, command, strlen(command), 0);
          break;
        ...
        case 'a': {
          ...
          resp = client->recv<op::ASCOMInterface::SwpGetGridPerRevolution,
                              uint32_t>();
          klog << "InquireGridPerRevolution Axis" << HEX(buffer[2]) - 1 << " " << resp
               << std::endl;
          synta_assembleResponse(command, buffer[1], resp);
          send(new_socket, command, strlen(command), 0);
          break;
        ...
        case 'F': {
          ...
          klog << "Initialize Axis" << HEX(buffer[2]) - 1 << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        case 'L': {
          ...
          klog << "InstantAxisStop Axis" << HEX(buffer[2]) - 1 << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        case 'K': {
          ...
          resp = client->recv<op::ASCOMInterface::SwpCmdStopAxis, bool>();
          klog << "NotInstantAxisStop Axis" << HEX(buffer[2]) - 1 << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        ...
        case 'G': {
          uint16_t cmd = Revu24str2long(buffer);
          isForward[axis] = (buffer[4] != '0') ? false : true;
          isSlew[axis] = (((buffer[3] - '0') & 0x1)  == 0) ? false : true;
          client->call<op::ASCOMInterface::SwpSetMotionModeDirection>(axis, isForward[axis], isSlew[axis], false);
          bool resp = client->recv<op::ASCOMInterface::SwpSetMotionModeDirection, bool>();
          klog << "SetMotionMode Axis" << HEX(buffer[2]) - 1 << " " << cmd  << " "  << buffer << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        ...
        case 'V': {
          ...
          klog << "SwpSetPolarScopeLED Axis" << HEX(buffer[2]) - 1 << " " << cmd << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        case 'I': {
          ...
          uint32_t min = client->recv<op::ASCOMInterface::get_min_period_ticks, uint32_t>();
          if (cmd < min ) cmd = min;
          client->call<op::ASCOMInterface::SwpSetStepPeriod>(axis, isSlew[axis], cmd);
          bool resp = client->recv<op::ASCOMInterface::SwpSetStepPeriod, bool>();

          klog << "SetStepPeriod Axis" << buffer << " " << cmd << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        case 'J': {
          client->call<op::ASCOMInterface::SwpCmdStartMotion>(axis, isSlew[axis], true, isIncrement[axis]);
          bool resp = client->recv<op::ASCOMInterface::SwpCmdStartMotion, bool>();
          klog << "StartMotion Axis" << HEX(buffer[2]) - 1 << " " << resp<< std::endl;
        ...
        case 'H': {
          ...
          if (resp) isIncrement[axis] = false;
          klog << "SetGotoTargetIncrement Axis" << HEX(buffer[2]) - 1 << " " << cmd << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
        case 'S': {
          ...
          if (resp) isIncrement[axis] = true;
          klog << "SetGotoTarget Axis" << HEX(buffer[2]) - 1 << " " << cmd << std::endl;
          synta_assembleResponse(command, resp == 0 ? '\0' : buffer[1], 0);
          send(new_socket, command, strlen(command), 0);
          break;
        }
